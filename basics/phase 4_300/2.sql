...
BEGIN
    ...
    INSERT INTO orders (id, user_id, total_amount, status, created_at) VALUES (gen_random_uuid(), v_user_id, v_amount, 'PENDING', NOW());
END $$;
...
BEGIN
    SELECT status INTO v_current_status FROM users WHERE id = v_user_id;
...
BEGIN
    ...
    INSERT INTO post (id, user_id, title, status, created_at) VALUES (gen_random_uuid(), v_user_id, 'My first post', 'DRAFT', NOW());
END $$;
...
DECLARE
    post_record RECORD;
BEGIN
    FOR post_record IN SELECT id, title FROM posts WHERE status = 'DRAFT' AND created_at < NOW() - INTERVAL '30 days'
    LOOP
        UPDATE posts SET status = 'ARCHIVED' WHERE id = post_record.id;
...
BEGIN
    ...
    IF v_user_status != 'ACTIVE' THEN RAISE EXCEPTION 'User is not active. Current status: %', v_user_status;
    END IF;
