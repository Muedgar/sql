...
-- IF / ELSE: activate or deactivate user
DO $$
DECLARE
    v_user_id UUID := 'USER_UUID_HERE';
...
-- VALIDATE USERS EXISTS BEFORE CREATING POST
DO $$
DECLARE
    v_user_id UUID := 'USER_UUID_HERE';
...
-- LOOP AND UPDATE MANY POSTS SAFELY
DO $$
DECLARE
    post_record RECORD;
BEGIN
...
-- RAISE EXCEPTION IF TOO MANY ROWS WILL BE UPDATED
DO $$
DECLARE
...
-- FULL VALIDATION SCRIPT BEFORE INSERTING ORDER
DO $$
DECLARE
    v_user_id UUID := 'USER_UUID_HERE';
...
BEGIN
    ...
    IF v_user_status != 'ACTIVE' THEN
        RAISE EXCEPTION 'User is not active. Current status: %',
        v_user_status;
    END IF;
